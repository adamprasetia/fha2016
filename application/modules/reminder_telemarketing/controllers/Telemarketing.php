...
class Telemarketing extends MY_Controller {
	public function __construct(){
		...
		$this->load->model('reminder_telemarketing/telemarketing_model');
		$this->load->model('reminder_telemarketing/callhis_model');
	}
	public function index(){
		...
		$xdata['action'] = 'reminder_telemarketing/telemarketing/search'.get_query_string();
		...
		foreach($head_data as $r => $value){
			$heading[] = anchor('reminder_telemarketing/telemarketing'.get_query_string(array('order_column'=>"$r",'order_type'=>$this->general->order_type($r))),"$value ".$this->general->order_icon("$r"));
		}		
		$heading[] = 'Action';
		...
		foreach($result as $r){
			...
			$this->table->add_row(
				$i++,
				anchor('reminder_telemarketing/telemarketing/phone/'.$r->id.get_query_string(),$r->sn).($r->valid==1?' <span class="label label-success">Valid</span>':'').($r->audit==1?' <span class="label label-primary">Audit</span>':''),
				$r->name,
				$r->title,
				$r->company,
				$r->tlp,			
				...
				anchor('reminder_telemarketing/telemarketing/phone/'.$r->id.get_query_string(),'Phone'.($count_call > 0?' <span class="label label-success">'.$count_call.' <span class="glyphicon glyphicon-earphone"></span></span>':''))
			);
		}
	...
	public function search(){
		...
		redirect('reminder_telemarketing/telemarketing'.get_query_string($data));		
	}	
