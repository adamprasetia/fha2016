...
class Export extends MY_Controller {
	public function __construct(){
		...
		$this->load->model('reminder_export/export_model');
		$this->load->model('telemarketing/callhis_model');
	}
	public function index(){
		$data['content'] = $this->load->view('reminder_export/export','',true);
	...
	public function execute(){		
		...
		if($this->form_validation->run()===false){
			...
			$active_sheet->setCellValue('J1', 'New Telephone');
			$active_sheet->setCellValue('K1', 'Distribution Date');
			$active_sheet->setCellValue('L1', 'Status');
			$active_sheet->setCellValue('M1', 'Call History');
			...
			foreach($result as $r){
				...
				$active_sheet->setCellValueExplicit('J'.$i, $r->tlp_new);
				$active_sheet->setCellValue('K'.$i, PHPExcel_Shared_Date::PHPToExcel(date_to_excel($r->dist_date)));
				$active_sheet->getStyle('K'.$i)->getNumberFormat()->setFormatCode('dd/mm/yyyy');		   
				$active_sheet->setCellValue('L'.$i, $r->status_name);
				$active_sheet->setCellValue('M'.$i, $this->callhis_model->get_note($r->id));
				$i++;
			}

			$filename='Reminder_Candidate_'.date('Ymd_His').'.xlsx';
